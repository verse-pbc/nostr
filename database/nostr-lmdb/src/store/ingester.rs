    ...
    fn process_operation(
        ...
        match item.operation {
            ...
            IngesterOperation::Delete { filter, tx } => {
                // Delete implementation that works with borrowing rules
                let result = (|| -> Result<(), Error> {
                    // First, query and collect all matching events as owned data
                    let events_to_delete: Vec<Event> = self.db.query(txn, filter.clone())?
        ...
        match self.db.write_txn() {
            Ok(mut txn) => {
                ...
                for item in &mut batch_iter {
                    let result = self.process_operation(&mut txn, item, fbb);
                    
                    // Only abort on actual database errors, not on rejections
                    let should_abort = match &result {
                        OperationResult::Save { result: Err(_), .. } | 
                        OperationResult::Delete { result: Err(_), .. } => true,
                        OperationResult::Save { result: Ok(SaveEventStatus::Rejected(_)), .. } => false, // Rejections are expected, don't abort
                        _ => false,
                    };
                ...
                if has_error {
                    ...
                    // Create error once and reuse
                    let abort_error = Error::Other("Transaction aborted due to batch error".to_string());
                    
                    // Convert all previously processed results to abort error
                    for result in results.iter_mut() {
                        match result {
                            OperationResult::Save { result: res, .. } => *res = Err(abort_error.clone()),
                            OperationResult::Delete { result: res, .. } => *res = Err(abort_error.clone()),
                        }
                    }
                    
                    // Send abort errors for remaining unprocessed items
                    for item in batch_iter {
                        let error_result = item.operation.into_error_result(abort_error.clone());
                        ...
                        Err(e) => {
                            ...
                            for result in results.iter_mut() {
                                match result {
                                    ...
                                    OperationResult::Delete { result: res, .. } => *res = Err(error.clone()),
                                }
                            }
                        }
                    }

...
use nostr_database::{
    Backend, DatabaseError, DatabaseEventStatus, Events, FlatBufferBuilder, NostrDatabase,
    SaveEventStatus,
};
...
impl NostrLmdbBuilder {
    ...
    pub fn build(self) -> Result<NostrLMDB, DatabaseError> {
        ...
        let max_dbs: u32 = self.max_dbs.unwrap_or(20);
...
impl NostrLMDB {
    ...
    pub fn write_transaction(&self) -> Result<heed::RwTxn<'_>, DatabaseError> {
        self.db.write_transaction().map_err(DatabaseError::backend)
    }

    /// Create a new read transaction
    pub fn read_transaction(&self) -> Result<heed::RoTxn<'_, heed::WithTls>, DatabaseError> {
    ...
    pub fn save_event_with_txn(
        ...
        self.db
            .save_event_with_txn(txn, fbb, event)
            .map_err(DatabaseError::backend)
    }
...
mod tests {
    ...
    async fn test_expected_query_result() {
        ...
        for (i, event_str) in EVENTS.into_iter().enumerate() {
            ...
            println!("Event {}: {} - Kind: {:?}, Status: {:?}", i, event.id, event.kind, status);
        ...
        println!("Actual has {} events, expected has {} events", actual.len(), expected_output.len());
